...
export async function POST(req: Request) {
  try {
    ...
    const rule = getRateLimitRule("signup");
    const rateCheck = await checkRateLimit(`signup:${ip}`, rule.limit, rule.windowSec);
    ...
    return createSafeAuthErrorResponse(err, AUTH_MESSAGES.GENERIC_ERROR, "api/auth/signup");
  }
}
