...
//
// WHY: Prompts like "build me a full stack app" or "write all the code"
// ask mentors to generate entire codebases rather than teach or advise.
// This creates: (a) extreme token consumption, (b) low-quality output since
// a mentor can't build a complete app in 800 tokens, and (c) creates
// dependency rather than learning. We reject these at the API layer before
...
export function detectScopeViolation(text: string): {
  ...
  if (text.length > 1200) {
    const hasImplementIntent = [
      "build me", "create me", "make me", "implement this",
      "write this for me", "code this", "generate this",
    ].some((p) => q.includes(p));
  ...
  if (numberedSteps >= 5 && (q.includes("implement") || q.includes("build") || q.includes("create"))) {
    return {
      violated: true,
